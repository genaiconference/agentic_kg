...
Health_Special_Instructions,
Leave_Special_Instructions,
...
health_insurance_tool = create_retriever_tool(
    ...
    name="HealthInsuranceTool",
    description="Use this tool to answer questions specifically about the company's health insurance policy"
)
...
def combined_answer_node(state):
    print("------ENTERING: ANSWER NODE------")
    tools = [graph_tool, adonis_graph_retriever_tool, health_insurance_tool, leave_policy_tool]
    generate_agent = get_react_agent(
        llm,
        tools,
        COMBINED_DG_REACT_PROMPT,
...
def _create_graph_builder():
    ...
    builder.add_node("answer_node", combined_answer_node)
